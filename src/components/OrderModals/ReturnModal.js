...
const ReturnModal = ({ showReturnModal, setShowReturnModal, productsData, handleReturn, returnLoading=false }) => {
    ...
    return (
        <Modal
            ...
            title={<>
                <Title level={4}>Return {items} items</Title>
                <Text>Select items that are coming back</Text>
            </>}
            onCancel={() => setShowReturnModal(false)}
            footer={[
                <Button size='large' onClick={() => setShowReturnModal(false)}>Cancel</Button>,
                selectedRowKeys.length > 0 ?
                    <Button
                    style={{ background: 'rgb(116, 204, 32)' }}
                        loading={returnLoading}
                        icon={<FaLevelDownAlt />} type='primary' size='large' onClick={()=>handleReturn(selectedId, 'returned')}>
                        Return {items} items
                    </Button>
                    :
                    <Button
                        icon={<FaLevelDownAlt />} size='large'>
                        No items to return
                    </Button>
            ]}
        >
